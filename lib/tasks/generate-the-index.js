...
module.exports = function generateTheIndex (ctx, cb) {
    ...
    $("script[type='application/webspec+json']")
        .each(function generateTheIndexSeekHandler () {
            var $script = $(this)
            ,   ws = JSON.parse($script.text())
            ,   $dl = $("<dl class='webspec'></dl>")
            ,   simpleDD = function generateTheIndexDdHonk (desc, $dl) {
                    $dl.append($("<dd></dd>").html(desc || "n/a"));
                }
            ,   linkDT = function generateTheIndexDtHonk (repo, $dl) {
    ...
    $("section")
        .each(function generateTheIndexSectionHonk () {
            var $section = $(this)
    ...
    ;
    var tocline = function generateTheIndexTocLineHonk (details, $parent) {
        var $li = $("<li><a></a></li>")
        ,   $a = $li.find("a")
        ;
