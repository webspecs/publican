...
exports.bikeshedRepositories = function (repos, conf, cb) {
    ...
    async.each(
        exports.canonical2pairs(repos)
    ,   function (target, cb) {
            bs.bikeshed(conf.python, jn(dataDir, "bikeshed/bikeshed.py"), target.publishDir, target.fileName, cb);
        }
    ,   cb
    );
};
    ...
    async.series(
        ...
        ,   function (cb) {
                if (regen === "all") {
                    ...
                    bs.bikeshed(conf.python, jn(dataDir, "bikeshed/bikeshed.py"), jn(dataDir, "publish", wanted[repo].branches[branch]), wanted[repo].fileName, cb);
                }
            }
